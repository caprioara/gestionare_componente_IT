...
async function submit() {
    ...
    if (user?.success) window.location.replace('http://127.0.0.1:5500/templates/products.html');
}
...
async function lmao(){
    ...
    for(let i = 0; i < rezultat.data.length; i++){
        ...
        <div class="col-md-3 mt-1"><img id="produs1" class="img-fluid img-responsive rounded product-image" src="${rezultat.data[i].image}">
        </div>
...
async function sendData(e){
    ...
    if(image !== '' && price !== '' && stock !== '' && name !== '' && category !=='' ){
